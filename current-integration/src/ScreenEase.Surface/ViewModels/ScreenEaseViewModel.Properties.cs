...
{
    private ScreenEaseServiceUnitStatus? _serviceUnitStatus;
    private string _serviceUnitSummary = "";

    /// <summary>
    /// Live status of the ScreenEase.Service unit (null when no ServiceManager is supervising it).
    /// Folded compactly into the diagnostics/title area, not a large generic status bar.
    /// </summary>
    public ScreenEaseServiceUnitStatus? ServiceUnitStatus
    {
        get => _serviceUnitStatus;
        set
        {
            if (SetProperty(ref _serviceUnitStatus, value))
            {
                ServiceUnitSummary = value?.Summary ?? "";
                OnPropertyChanged(nameof(HasServiceUnit));
                OnPropertyChanged(nameof(ServiceUnitRunning));
            }
        }
    }

    public string ServiceUnitSummary
    {
        get => _serviceUnitSummary;
        private set => SetProperty(ref _serviceUnitSummary, value);
    }

    public bool HasServiceUnit => _serviceUnitStatus is not null;
    public bool ServiceUnitRunning => _serviceUnitStatus?.IsRunning ?? false;
